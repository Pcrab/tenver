...
def video_thread(n):
    ...
    zoom = [
        ...
        ("./cv/video/test4.mp4",
         np.array([[310, 336], [639, 238], [638, 225], [362, 190], [0, 227], [0, 253]]),
    ...
    out1 = cv2.VideoWriter(path + "output1.mp4", fourcc, 20.0, (640, 480))  # 分辨率要和原视频对应
    out2 = cv2.VideoWriter(path + "output2.mp4", fourcc, 20.0, (640, 480))  # 分辨率要和原视频对应
    ...
    while START:
        ...
        if len(realContours) == 0:
            ...
            cv2.rectangle(fgmask, (rect[0], rect[1]), (rect[0] + rect[2], rect[1] + rect[3]), (0xff, 0xff, 0xff),
                          1)  # 黑白前景上绘制矩形
            M = cv2.getPerspectiveTransform(zoom[video][1].astype(np.float32), transform)
            if len(oldPosition) != 0:
                k = (oldPosition[0] - position[0]) * (oldPosition[1] - position[1])
                ...
                if k > 0 and oldK < 0:
                    ...
                    if frameNum - previousYes > 50:
                        ...
                        dst = cv2.imread(imgName)
                        cv2.circle(dst, (int(transf_point[0]), int(transf_point[1])), 2, (0, 0, 255), -1)
...
PORT = 4200
server = WebsocketServer(port=PORT, loglevel=logging.INFO)  # 创建Websocket Server
